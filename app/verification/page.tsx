...
const SERVER_REQUIREMENTS = [
  ...
  { title: "Safe for work", description: "The community and its public Nexus listing must remain SFW.", icon: "solar:heart-angle-linear" },
] as const;
...
const PROCESS = [
  { title: "Nexus checks your server or bot", description: "Nexus reviews trusted platform signals, listing information, activity, and safety indicators to confirm that the project meets the baseline requirements.", icon: SearchCheck },
  { title: "Review and agree to the terms", description: "The lister reviews and accepts the Nexus verification terms and conditions, including the ongoing safety and maintenance responsibilities.", icon: FileCheck2 },
  { title: "Receive the verification badge", description: "After the platform checks and terms are completed, Nexus issues the badge to the eligible listing and its lister profile.", icon: Award },
] as const;

const FAQS = [
  { id: "apply", question: "Can I apply for verification?", answer: "Not yet. Nexus monitors eligibility automatically. When a listing appears eligible, its lister will receive a notification with the next steps." },
  ...
  { id: "guaranteed", question: "Does meeting every requirement guarantee verification?", answer: "No. Requirements make a listing eligible for review, but Nexus may consider authenticity, safety history, platform abuse, maintenance quality, and other trust signals." },
  { id: "removed", question: "Can a verification badge be removed?", answer: "Yes. Nexus may remove verification if a server, bot, or lister stops meeting the requirements or violates Discord or Nexus policies." },
  { id: "benefits", question: "What are the benefits of verification?", answer: "Verification adds a trusted identity signal, can improve discovery confidence, and may unlock future creator and developer features as Nexus grows." },
] as const;
      ...
      <section className="relative overflow-hidden border-b border-border px-4 py-16 md:px-6 md:py-24 lg:py-28">
        <div className="relative mx-auto grid w-full max-w-[1240px] items-center gap-12 lg:grid-cols-[minmax(0,0.9fr)_minmax(520px,1.1fr)]">
          <div className="max-w-2xl">
            <Chip color="accent" variant="soft"><VerifiedBadgeIcon className="size-4 text-accent" /><Chip.Label>Nexus Verification</Chip.Label></Chip>
            <h1 className="mt-5 text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Trust that stands out.</h1>
            <p className="mt-5 max-w-xl text-base leading-relaxed text-muted sm:text-lg">Nexus verification helps people recognize established, authentic, and safely maintained Discord servers and bots.</p>
            ...
            <p className="mt-5 flex items-center gap-2 text-sm text-muted"><Clock3 className="size-4 text-accent" />Verification is awarded by Nexus. It cannot be selected while creating a listing.</p>
          </div>
        ...
        <section id="requirements" className="scroll-mt-24 px-4 py-16 md:px-6 md:py-24">
          <div className="mx-auto w-full max-w-[1120px]">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h2 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">What Nexus looks for</h2>
        ...
        <section id="process" className="scroll-mt-24 border-y border-border bg-default/20 px-4 py-16 md:px-6 md:py-24">
          <div className="mx-auto w-full max-w-[1040px]">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 leading-relaxed text-muted">Nexus checks listings proactively. There is currently no public application form or paid shortcut.</p>
            </div>
          ...
          <div className="mx-auto w-full max-w-[1120px]">
            <div className="grid gap-5 lg:grid-cols-2">
              <Card className="nexus-card gap-4 p-6">
                ...
                <div><Card.Title>Verified lister profile</Card.Title><Card.Description className="mt-2 leading-relaxed">The person or team responsible for an eligible listing receives a verification badge beside their Nexus profile identity.</Card.Description></div>
  ...
  return (
    <div ref={ref} className={`verification-timeline-step relative grid min-h-[52vh] grid-cols-[3rem_minmax(0,1fr)] items-center gap-4 py-8 sm:grid-cols-[3.5rem_minmax(0,1fr)] md:min-h-[58vh] ${visible ? "is-visible" : ""}`}>
      ...
      <Card className="verification-timeline-card nexus-card min-h-56 justify-center gap-5 p-6 md:min-h-64 md:p-8">
        ...
        <p className="text-xs font-medium text-muted">{index === PROCESS.length - 1 ? "The verified status is now visible on Nexus." : "Continue scrolling to see the next step."}</p>
      </Card>
    </div>
  );
}
