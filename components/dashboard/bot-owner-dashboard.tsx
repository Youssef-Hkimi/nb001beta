...
const METRICS = [
  ...
  { key: "votes", label: "Votes", icon: ThumbsUp, color: "text-purple-400" },
] as const;
...
export function BotOwnerDashboard() {
  ...
  return (
    <div id="bots" className="scroll-mt-28 space-y-8">
      ...
      <AnalyticsLineChart
        title="Server growth"
        description="Track reported server additions and removals over time."
        history={bot.analytics.history}
        series={[
          { key: "newServers", label: "New Servers", color: "#34D399" },
          { key: "removedServers", label: "Removed Servers", color: "#F87171" },
        ]}
      />
...
function GrowthRow({ label, value, tone }: { label: string; value: number; tone?: "positive" | "negative" }) { return <div className="flex items-center justify-between rounded-xl border border-border px-4 py-3"><span className="text-sm text-muted">{label}</span><span className={`font-bold ${tone === "positive" ? "text-emerald-500" : tone === "negative" ? "text-red-400" : "text-foreground"}`}>{tone === "positive" ? "+" : tone === "negative" ? "−" : ""}{formatCount(value)}</span></div>; }
